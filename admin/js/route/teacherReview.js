...
$('#j-ok').click(function(){
    if($('#complete-dialog').attr('data-g')==1){
        var obj={
            'id':$(this).parents('#complete-dialog').attr('data-id'),
            'status':'0',
            'text':$('#j-text').val()
        }
    }
...
function getHtml(data){
    for(var i=0;i<data.length;i++){
        if(+data[i].status==200){
            data[i].deal='<span  style="cursor: pointer" class="btn-block text-info j-n-r"> 复核 </span></li><span style="cursor: pointer" class="btn-block text-danger j-y-r"> 驳回 </span></li>';
        }
