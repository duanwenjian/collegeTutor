...
$sqlu="SELECT `username`, `password` FROM `ct_user` WHERE email='$email'";
$resu=mysql_query($sqlu);
$row4=mysql_num_rows($resu);
$rowsu=mysql_fetch_array($resu);

if($row4==1){
    $token = md5($rowsu['username'].$rowsu['password'].$regtime); //创建用于激活识别码
    $token_exptime = time()+60*60*24;//过期时间为24小时后
    $sql = "UPDATE `ct_user` SET `token`='$token',`token_exptime`='$token_exptime' WHERE email='$email'";
    mysql_query($sql);
    if(mysql_affected_rows()){//写入数据库成功，发邮件


        include 'smtp.class.php';
        $mailto=$email; 	 //收件人
        $subject="大学生家教网用户重置密码"; 			 //邮件主题
        $body="<div style='background-color: rgba(39, 174, 91, 0.3);padding: 10px;font-size: 14px;line-height: 18px'><div style='background-color: #efefef;padding: 6px'><div style='border: 3px solid rgba(39, 174, 91, 0.3);padding: 20px;border-radius: 3px;color: #3a3a3a'><p style='margin: 0;font-size: 20px'>尊敬的用户：<span style='color:rgba(39, 174, 91, 1);font-weight: bold;'>".$rowsu['username']."</span></p><p style='text-indent: 30px;margin: 5px'>您好！</p><div ><p style='margin: 3px'>请点击下面链接更改您的帐号登陆密码。</p><p style='margin: 8px;'><a href='".$addr."/forgot.html?verify=".$token."&email=".$email."' target='_blank'>".$addr."/forgot.html?verify=".$token.'&email='.$email."</a></p><p style='margin: 3px'>如果以上链接无法点击，请将它复制到你的浏览器地址栏中进入访问，该链接24小时内有效。</p><p style='margin: 3px 0 10px 0'>如果此次激活请求非你本人所发，请忽略本邮件。</p><p style='text-align:right;margin: 3px'>&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45; <a href='".$addr."'>大学生家教网 敬上</p><p style='margin: 3px;text-align: right'>请勿直接回复本邮件</p></div></div></div></div>";  //邮件内容
