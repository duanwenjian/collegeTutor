...
//
$('body').on('click','.j-n-2',function(){
    var obj={
        'userID':$(this).parents('tr').attr('data-id')
    }
    $.ajax({
        url:'php/userAdminDelete.php',
        type:'post',
        dataType:'json',
        data:obj,
        success:function(data){
...
function getHtml(data){
    for(var i=0;i<data.length;i++){
        data[i].deal='<button type="button" class="btn pmd-btn-flat pmd-ripple-effect btn-info j-n-2"> 移除管理 </button>';
